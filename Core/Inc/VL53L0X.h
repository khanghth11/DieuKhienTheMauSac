...
//------------------------------------------------------------
// For quick and dirty C++ compatibility & standard types
//------------------------------------------------------------
#include <stdbool.h> // Use standard boolean types
#include <stdint.h>
#include "stm32f4xx_hal.h" // Include HAL for I2C_HandleTypeDef

// #define bool  uint8_t // No longer needed with stdbool.h
// #define true  1
// #define false 0
...
// Moved g_timeoutStartMs to .c file as static
// #define startTimeout() (g_timeoutStartMs = HAL_GetTick())

// Check if timeout is enabled (set to nonzero value) and has expired
// Moved g_ioTimeout and g_timeoutStartMs to .c file as static
...
// Iniitializes and configures the sensor using the provided I2C handle.
// If the optional argument io_2v8 is true, the sensor is configured for 2V8 mode (2.8 V I/O);
// if false, the sensor is left in 1V8 mode. Returns true if the initialization completed successfully.
// *** MODIFIED: Takes I2C_HandleTypeDef pointer ***
...
// Returns true if the requested limit was valid.
...
// Sets the VCSEL pulse period for the given period type. Returns true if valid.
bool setVcselPulsePeriod(vcselPeriodType type, uint8_t period_pclks);

// Returns the current VCSEL pulse period for the given period type.
uint8_t getVcselPulsePeriod(vcselPeriodType type);

// Starts continuous ranging measurements. period_ms = 0 for back-to-back mode.
...
// Returns 65535 on timeout. Check timeoutOccurred() after reading.
...
// Sets a timeout period in milliseconds. A value of 0 disables the timeout.
...
// Internal structures (moved from header to C file if only used internally,
// but kept here if needed by public functions or macros)
typedef struct {
